...
@Priority(Priorities.AUTHORIZATION) public class AuthorizationRequestFilter implements ContainerRequestFilter {
        ...
        if (!request.getUriInfo().getPath().matches(unAuthenticatedPathPattern)) {
            ...
            if (!request.getMethod().equals(HttpMethod.OPTIONS)) {
                ...
                if (info != null) {
                    ...
                    checkPublicAccess(publicAccessService.getDomainPublicRules(domainId), request, domainId);
                }
            }
        }
    }
    ...
    public void checkTokenAccessRules(final AuthorizationInfo info, final ContainerRequestContext request, String domainId) {
        Set<JsonObject> applicableRules = checkAccessRules(info.getAccessRules(), request, domainId, info.getUserId() != null);
    ...
    public void checkPublicAccess(Set<JsonObject> accessRules, final ContainerRequestContext request, String domainId) {
        Set<JsonObject> applicableRules = checkAccessRules(accessRules, request, domainId, false);
        // If no rules apply then by default access is denied
        if (applicableRules.isEmpty()) {
    ...
    public Set<JsonObject> checkAccessRules(Set<JsonObject> accessRules, final ContainerRequestContext request, String domainId, boolean userToken) {
